DROP procedure IF EXISTS `INV_UpdateLotPrice`;

DELIMITER $$

CREATE PROCEDURE `INV_UpdateLotPrice` (IN lotIDs VARCHAR(255), IN newValue INT, IN user VARCHAR(128))
BEGIN
...
START TRANSACTION; 
	...
	do_this:
		loop	
			...
			SET @prevValue = (SELECT price FROM inv_cur_lots WHERE id = lotID);
